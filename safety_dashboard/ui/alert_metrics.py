...
def render_alert_metrics(*, standalone: bool = False) -> None:
    container = st.container() if standalone else st.expander(
        "자동 알림 실적", expanded=False
    )
    with container:
        base_url = secret(
            "alerting",
        ...
        if not st.session_state.get("alert_metrics_loaded", False):
            ...
            return
        try:
            status = get_alert_json(base_url, "/internal/v1/notifications/status", token)
            metrics = get_alert_json(
                base_url,
        ...
        manual[0].metric(
            "수동 전파 성공",
            int(totals.get("telegram_manual_sent", 0)),
        )
        manual[1].metric(
            "수동 전파 실패",
            int(totals.get("telegram_manual_failed", 0)),
        )
        sms_activity = any(int(totals.get(key, 0)) for key in (
            "sms_attempted", "sms_accepted", "sms_delivered",
            "sms_delivery_failed", "sms_failed", "sms_unknown",
        ))
        if delivery_mode == "SMS 우선" or sms_activity:
            st.markdown("#### 문자 전달")
            second = st.columns(5)
        else:
            second = None
        ...
        _render_event_history(base_url, token, start, end)
        ...
        drill[0].metric(
            "훈련 전파",
            int(totals.get("manual_drill_sent", 0)),
        )
        drill[1].metric(
            "훈련 실패",
            int(totals.get("manual_drill_failed", 0)),
        )
        if mode == "미리보기":
...
def _render_event_history(
    ...
    status = filters[1].selectbox(
        ...
        key="notification-event-status",
    )
    limit = filters[2].selectbox(
        "표시 건수",
        options=(20, 50, 100),
        index=1,
        key="notification-event-limit",
    )
    try:
        payload = get_alert_json(
            base_url,
        ...
        st.warning(f"최근 전파 이력을 불러오지 못했습니다 ({type(exc).__name__}).")
        return
    events = payload.get("events", [])
    if not isinstance(events, list) or not events:
        st.info("선택한 기간과 조건에 해당하는 전파 이력이 없습니다.")
        return
    source_labels = {"automatic": "자동", "manual": "수동"}
    status_labels = {
        "SENT": "성공",
        "PENDING": "처리 대기",
        "RETRY_QUEUED": "재시도 중",
    ...
    for item in events:
        if not isinstance(item, dict):
            continue
        rows.append({
            "시각": _format_event_time(item.get("occurred_at")),
            ...
            "재시도·결과": str(item.get("detail", "")),
        })
    st.dataframe(
        rows,
        hide_index=True,
        width="stretch",
        height=min(520, 38 + 35 * len(rows)),
    )
    st.caption("전화번호·Telegram 식별자·관리자 토큰은 이력과 CSV에 저장하지 않습니다.")
