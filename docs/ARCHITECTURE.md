...
GET /api/v1/facilities/{facility_id}/context
    → 현재 날씨, 최근 재난문자, 인근 도로 CCTV와 각 데이터 상태
```

`monitoring` 응답만으로 검색·필터·지도·시설 기본 상세를 렌더링할 수 있어야
한다. `context`는 사용자가 시설을 선택하고 현장 참고정보를 요청했을 때만
호출한다. 날씨·재난문자·CCTV 중 일부가 실패해도 나머지 결과와 핵심 관제
응답을 유지하며 각 항목의 `LIVE`, `NOT_CONFIGURED`, `ERROR` 상태를 별도로
반환한다.
...
- 날씨·재난문자·CCTV는 자동으로 미리 조회하지 않고 선택 시설의 현장
  참고정보를 사용자가 요청할 때만 조회한다.
