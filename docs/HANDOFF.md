...
>
> 마지막 정리: 2026-08-28 10:22 KST
> 기준 Git: `main` / `a041ef5 Refine mobile field map legend`
...
개선`을 단계별 독립 커밋으로 진행 중이며 현재 진행률은 **20%**입니다.

- 완료: 현장 지도 범례 도움말의 불필요한 안전 주의 문구 제거, 모바일 범례를
  `위험등급 1행 + 특보구역 1행`으로 분리, 시설 미선택 안내판 높이 축소
- 검증: React Vitest 29개와 TypeScript·Vite 프로덕션 빌드 통과
- 제한: 이 세션에는 연결 가능한 브라우저가 없어 390px·430px 실제 화면 캡처는
  통합 검증 단계에서 다시 확인해야 함
- 보존: `output/`, `tmp/`는 untracked 상태 그대로이며 push·운영 배포는 하지 않음

**다음 작업 하나**: 10단계 2차 `중앙관제 운영 상황 압축`. `ControlApp.tsx`의 6개
버튼형 `.kpi-card`를 전체·영향 우선의 비조작형 요약 패널로 바꾸고, 상·중·하·
영향 없음은 한 줄 등급 요약으로 표시한다. 미판정·조회 불가는 0보다 클 때만
예외 항목으로 노출하며 집계값과 우선순위 목록 동작은 바꾸지 않는다.
