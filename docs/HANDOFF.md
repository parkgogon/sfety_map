...
- 완료: 전체 Python 단위 테스트 256개, React 70개 및 5단계 무결성 검증
  (`verify_all.sh`) 통과
...
- 완료 (모의훈련 단계 4): 종합 일치성(특보 4건, 영향 31개소, 상 10개소),
  ...
  현재 구현에 맞게 갱신했습니다.
- 완료 (모의훈련 지도 UX 개선 4단계 전체): [`SIMULATION_MAP_UX_IMPROVEMENT_PLAN.md`](SIMULATION_MAP_UX_IMPROVEMENT_PLAN.md)
  4개 단계를 전수 구현·검증했습니다.
  1) 1단계: 모의훈련 snapshot 도착 시 추천 레이어(태풍 시 바람) 1회 자동 선택 및 수동 선택 보존, 중앙관제 상단 배너 1곳으로 복귀 단일화, compact 범례 간결화(`바람 · 훈련값`).
  2) 2단계: `weatherLayerRendering.ts` 분리와 offscreen canvas 단일 alpha 합성을
  도입했습니다. 당시 방사형 격자 방식은 아래 진행 중인 개선 2단계에서 IDW 연속
  raster로 교체했습니다.
  3) 3단계: 화면 너비/축척 반응형 바람 spacing(48~120px) 및 22px edge inset 적용, 중심 거리·풍속 기반 100% 결정적 대표점 샘플링.
  4) 4단계: 실화면 매트릭스 검증, `UI_FLOW.md` 현행화, React 단위 테스트 57개 및 Python 256개 전수 통과.
- 진행 중 (지도 마커·기상 그래픽 시각 개선 1~3단계 완료, 4단계 자동·문서 검증 완료):
  [`MAP_VISUAL_REFINEMENT_PLAN.md`](MAP_VISUAL_REFINEMENT_PLAN.md)의 4단계 중
  1~3단계를 완료했습니다. 시설 103개 위치마다 기상 canvas를 지워 생기던 빈 구멍을
  제거하고 마커 z-index를 명시했습니다. 미선택 단일 시설은 등급과 관계없이
  30px, 같은 위치 숫자 마커 34px, 선택 시설 44px, 클러스터 38px로 정리했으며
  클러스터 기준은 level 9/48px로 한 단계 늦췄습니다. 기온·강수는 4px 저해상도
  IDW 연속 raster와 외곽 감쇠로 교체했고 강수 0mm 경계와 최대 120,000개 샘플을
  보장합니다. 바람도 `speed_ms` 보간 색면을 alpha 0.12로 화살표 아래에 추가하고
  화살표를 14~30px, 흰 외곽선 3.6px/alpha 0.78, 본선 2px로 정돈했습니다. 기존
  반응형 표본 간격과 22px 가장자리 보호는 유지했습니다. React 70개와 production
  build를 포함한 `scripts/verify_all.sh` 전체가 통과했습니다. 4단계에서 로컬
  실시간·모의훈련 현장지도와 중앙관제 네 경로의
  HTTP 200을 확인하고 `DESIGN_SYSTEM.md`, `UI_FLOW.md`를 현행화했습니다. 다만
  인앱 브라우저의 사용 가능 브라우저 목록이 비어 있어 PC·모바일 캡처와 지도
  클릭 검수는 수행하지 못했으며 완료로 표시하지 않았습니다. 구현 커밋 `40e0ddd`의
  GitHub Actions 실행 `33278837855`에서 테스트, Cloud Run API·Worker, Firebase
  Hosting, 가동상태 감시 설정이 모두 성공했습니다. 운영 `/`, `/control`,
  `/settings`는 HTTP 200과 `no-cache, no-store`였고, 2026-08-30 07:40 KST
  자동감시에서 Worker·KMA 상태가 모두 `live`였습니다.

**다음 작업 하나**: 브라우저 연결 또는 사용자 배포 화면 확인으로 위 계획 4단계의
실화면 매트릭스만 마친다. 현장지도·중앙관제, 실시간·모의훈련, PC·모바일, 지도
레벨 7~10에서 가독성·구멍 부재·레벨 8/9 클러스터 전환·이동/확대 redraw와 클릭을
확인한다. 회귀가 없으면 `HANDOFF.md`에 결과를 반영하고 완료된 임시 계획서를
정리한다.
...
- **지도 마커는 동일 크기, 색상으로 등급 구분**: 미선택 단일 시설은 위험등급과
  관계없이 같은 크기로 표시합니다. 같은 위치 시설·선택 시설·원거리 클러스터만
  숫자와 선택성을 위해 별도 크기를 사용하며, 일반 클러스터는 지도 level 9부터
  적용합니다.
- **기상 그래픽은 정적인 연속 기상장**: 기온·강수는 지도 가독성을 보존하는
  연속 색면, 바람은 옅은 풍속 색면과 정적 화살표로 표현합니다. Windy식 입자
  애니메이션은 현재 범위에 포함하지 않습니다.
...
- 지도 시각 개선 구현은 `40e0ddd`로 `main`에 push·배포됐습니다. 사용자의 운영
  PC·모바일 실화면 확인만 기다리고 있습니다.
