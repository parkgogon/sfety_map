...
>
> 마지막 정리: 2026-09-03 08:20 KST
> 기준 배포 Git: `main` (`85e096f`) / 사용성 및 UI/UX 개선(배너, 로그인 정렬, 기상 지연 안내, 모바일 카드뷰) 완료, 미커밋
...
- 완료 (모의훈련 단계 4): 종합 일치성(특보 4건, 영향 31개소, 상 10개소),
  ...
  현재 구현에 맞게 갱신했습니다.
- 완료 (모의훈련 지도 UX): 특보 기반 추천 기상 레이어, 수동 선택 보존, 중앙관제
  복귀 동작 단일화, 간결한 훈련 범례, 결정적 모의 기상값과 실시간 격리를
  구현·배포했습니다. 완료된 임시 계획서는 현재 상태 문서에서 제거했습니다.
- 완료 (지도 마커·연속 기상 그래픽): `40e0ddd`에서 시설 위치 구멍 제거,
  단일 마커 30px·선택 44px·클러스터 level 9 전환, 기온·강수 IDW 연속 색면,
...
- 완료 (사용성 및 UI/UX 개선 — 상단 배너, 로그인 정렬, 기상 지연 안내, 모바일 카드뷰):
  1. 모바일 현장 안전지도(`App.tsx`, `styles.css`): KMA 수신 지연 등 예외 안내 배너에 닫기(`×`) 버튼을 추가하여 지도 가시성을 확보하되, 상단 수신 상태 바의 `● KMA 수신 지연` 뱃지는 계속 유지해 위험 인지 불변을 보장했습니다.
  2. 위험도 설정 페이지(`SettingsApp.tsx`, `styles.css`): 데스크톱 접속 시 로그인 카드를 뷰포트 정중앙(`admin-login-wrapper`)에 단정하게 배치하고 폼과 그림자 여백을 모던화했습니다.
  3. 기상 실황 지연 인지 강화(`FacilitySheet.tsx`, `WeatherLayerControls.tsx`): 기상청 API 통신 지연 시 기계적 오류명(`ConnectTimeout` 등) 노출을 차단하고, 직전 성공 관측값이 있을 때 `⚠️ 실시간 수신 지연으로 직전(HH:MM) 관측값을 표시 중입니다` 경고와 함께 데이터를 유지하며, 최초 실패 시 친절한 재시도 안내를 제공합니다.
  4. 모바일 중앙관제 점검 우선순위 반응형 카드뷰(`ControlApp.tsx`, `styles.css`): 768px 이하 모바일 화면에서 가로 스크롤 테이블 대신 `[순위+위험등급] [시설명] [발효특보] [위치] [담당부서]`가 한눈에 들어오는 모바일 전용 카드 리스트로 자동 전환했습니다.
  5. 로드맵 보관(`IMPROVEMENT_ROADMAP.md`): 데스크탑 통합 관제 콕핏, 현장 스마트폰 PWA, 복합재난 가중치 뷰 등 목적 지향적 3대 구조 개편 과제를 정식 백로그에 기록·보관했습니다.
  6. 단위 테스트(`App.test.tsx`, `FacilitySheet.test.tsx`, `ControlApp.test.tsx` 115개) 및 전체 무결성 검증(`scripts/verify_all.sh`) 5단계 전수 통과를 완료했습니다.

**다음 작업 하나**: 구현 및 검증된 사용성·UI/UX 개선사항을 검토하고 커밋, 푸시 및 운영 배포를 진행한다.
...
- 모의훈련은 특보·위험등급과 연관된 결정적 훈련 기상값을 사용하고,
  실제 관측이 아님을 명시합니다.
...
- **지도 마커는 동일 크기, 색상으로 등급 구분**: 미선택 단일 시설은 위험등급과
  ...
  적용합니다.
- **기상 그래픽은 지도 이동과 동기화**: 기온·강수 연속 색면은 유지하되 panning
  중 숨기지 않고 지도와 같은 방향·속도로 이동시킵니다. 바람 파티클도 같은 canvas
  이동 좌표계를 사용하며 zoom 중에는 일시 중지 후 재투영할 수 있습니다.
