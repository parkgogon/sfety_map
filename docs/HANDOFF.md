...
>
> 마지막 정리: 2026-08-30 07:30 KST
> 기준 배포 Git: `main` (`023be51`) / 모의훈련 지도 UX 개선 (단계 1~4) 배포 완료
...
- 진행 중 (지도 마커·기상 그래픽 시각 개선 1~3단계 완료, 4단계 자동·문서 검증 완료):
  ...
  build를 포함한 `scripts/verify_all.sh` 전체가 통과했고 아직 커밋·push·배포하지
  않았습니다. 4단계에서 로컬 실시간·모의훈련 현장지도와 중앙관제 네 경로의
  HTTP 200을 확인하고 `DESIGN_SYSTEM.md`, `UI_FLOW.md`를 현행화했습니다. 다만
  인앱 브라우저의 사용 가능 브라우저 목록이 비어 있어 PC·모바일 캡처와 지도
  클릭 검수는 수행하지 못했으며 완료로 표시하지 않았습니다.
...
정리한다. 커밋·push·배포는 별도 요청 전까지 하지 않는다.
...
- 2026-08-30 지도 시각 개선 1~3단계와 4단계 자동·문서 검증 변경이 working tree에
  있으며 아직 커밋·push·배포하지 않았습니다.
