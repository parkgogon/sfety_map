...
>
> 마지막 정리: 2026-08-28 10:43 KST
> 기준 Git: `main` / `8e1f63d Add mobile risk policy cards`
...
개선`을 단계별 독립 커밋으로 진행 중이며 현재 진행률은 **85%**입니다.
...
- 완료: 대상 분석·전파의 실제 CSS 선택자를 바로잡고, 모바일은 시설 선택 카드
  목록을 지도보다 먼저 표시. PDF와 수동 Telegram은 공통 선택 범위를 사용
- 완료: 위험도 정책의 데스크톱 행렬표를 유지하면서 700px 이하에서 14개 특보별
  카드로 전환. `주의보·경보·중대` 3열과 한글 등급만 표시하고 API 값은 유지
- 검증: React Vitest 34개와 TypeScript·Vite 프로덕션 빌드 통과
- 제한: 이 세션에는 연결 가능한 브라우저가 없어 390px·430px 실제 화면 확인은
  통합 검증 단계에서 재시도해야 함
- 보존: `output/`, `tmp/`는 untracked 상태 그대로이며 push·운영 배포는 하지 않음

**다음 작업 하나**: 10단계 5차 `통합 검증과 최종 인수인계`. `scripts/verify_all.sh`을
실행하고 390px·430px·데스크톱에서 현장 범례, 운영 요약, 대상 선택·PDF·수동 전파,
정책 카드를 확인한다. `UI_FLOW.md`와 `DESIGN_SYSTEM.md`를 실제 반응형 구조로 갱신하고,
브라우저 미연결로 실기 검증이 막히면 그 항목만 미완료로 명확히 남긴다.
...
완료됐고, 10단계 모바일 UI 밀도 개선은 4/5 완료 상태입니다.
