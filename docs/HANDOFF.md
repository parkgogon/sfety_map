...
>
> 마지막 정리: 2026-08-28 10:26 KST
> 기준 Git: `main` / `21e3ddf Compact control overview on mobile`
...
[`IMPROVEMENT_ROADMAP.md`](IMPROVEMENT_ROADMAP.md)의 `10단계 — 모바일 UI 밀도
개선`을 단계별 독립 커밋으로 진행 중이며 현재 진행률은 **40%**입니다.

- 완료: 현장 지도 범례 도움말의 불필요한 안전 주의 문구 제거, 모바일 범례를
  `위험등급 1행 + 특보구역 1행`으로 분리, 시설 미선택 안내판 높이 축소
- 완료: 중앙관제 운영 상황의 6개 버튼형 KPI를 전체·영향 우선의 비조작형 요약
  패널로 교체하고, 등급 4개를 한 줄로 압축. 미판정·조회 불가는 있을 때만 표시
- 검증: React Vitest 30개와 TypeScript·Vite 프로덕션 빌드 통과
- 제한: 이 세션에는 연결 가능한 브라우저가 없어 390px·430px 실제 화면 캡처는
  통합 검증 단계에서 다시 확인해야 함
- 보존: `output/`, `tmp/`는 untracked 상태 그대로이며 push·운영 배포는 하지 않음

**다음 작업 하나**: 10단계 3차 `대상 분석·전파 모바일 개선`. `ControlApp.tsx`의
실제 클래스(`analysis-action-bar`, `selection-buttons`, `action-buttons`)와 현재 잘못된
모바일 CSS 선택자를 맞추고, 모바일에서는 시설 선택 목록을 지도보다 먼저 배치한다.
데스크톱 표는 유지하면서 모바일 전용 선택 카드를 추가하되 같은
`selectedFacilityIds`가 PDF와 수동 Telegram에 그대로 전달되게 한다.
...
개선 로드맵([`IMPROVEMENT_ROADMAP.md`](IMPROVEMENT_ROADMAP.md))의 1~9단계는
완료됐고, 10단계 모바일 UI 밀도 개선은 1/5 완료 상태입니다.
