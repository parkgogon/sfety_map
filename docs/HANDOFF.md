...
>
> 마지막 정리: 2026-08-28 16:48 KST
> 기준 Git: `main` / `10d84db fix: PDF 초동보고서 다운로드 시 전체 103개 소관시설 기준 정상 집계 및 카드 라벨 분기 처리`
...
A4 세로형 PDF 초동보고서의 전면 개편(와이드 지도 116mm, 지도 일체형 통합 범례 푸터, 우측 인포 패널, Noto Sans TTF 한글 폰트 분리, 전체 103개소 집계 보정) 및 공개 다운로드 전환이 완료되었습니다.

- 완료: PDF 보고서 다운로드 시 관리자 인증 제거(공개 다운로드 허용)
- 완료: TrueType NotoSansKR-Bold/Regular 정식 폰트 탑재(글자 굵기 및 선명도 대폭 개선)
- 완료: 와이드 지도(116mm) 및 거리 기반 겹침 방지 알고리즘, 지도 일체형 통합 범례 푸터 구현
- 완료: 중앙관제 헤더의 `PDF 초동보고서 다운로드` 실행 시 전체 소관시설 103개소 기준 종합보고서 정상 발행 및 집계 라벨 분기 처리
- 완료: 5단계 통합 검증(데이터 103개, Python 229개, React Vitest 34개, TS/Vite 빌드) 통과 및 배포 완료

- 완료 (1차): 현장 지도 범례 도움말의 불필요한 안전 주의 문구 제거, 모바일 범례를
  `위험등급 1행 + 특보구역 1행`으로 분리, 시설 미선택 안내판 높이 축소
- 완료 (2차): 중앙관제 운영 상황의 6개 버튼형 KPI를 전체·영향 우선의 비조작형 요약
  패널로 교체하고, 등급 4개를 한 줄로 압축. 미판정·조회 불가는 있을 때만 표시
- 완료 (3차): 대상 분석·전파의 실제 CSS 선택자를 바로잡고, 모바일은 시설 선택 카드
  목록을 지도보다 먼저 표시. PDF와 수동 Telegram은 공통 선택 범위를 사용
- 완료 (4차): 위험도 정책의 데스크톱 행렬표를 유지하면서 700px 이하에서 14개 특보별
  카드로 전환. `주의보·경보·중대` 3열과 한글 등급만 표시하고 API 값은 유지
- 완료 (5차 자동 검증): `UI_FLOW.md`와 `DESIGN_SYSTEM.md` 반응형 문서화,
  Firestore TTL 테스트 안정화, 원클릭 5단계 통합 검증(데이터 103개, Python
  226개, React Vitest 34개, TS/Vite 빌드) 통과
- 완료 (상태 복구): 프런트엔드 패키지를 잠금파일에서 검증한 정확한 버전으로
  고정하고, 단일 번들 전환 결정과 문서의 오래된 `React.lazy` 설명을 실제 코드에 맞춤
- 완료 (배포 설정): Firebase 헤더는 rewrite 전 요청 경로로 매칭되므로 `/`,
  `/control`, `/settings`에도 HTML 무캐시 규칙을 추가하고 회귀 테스트로 고정
- 보존: `output/`, `tmp/`는 untracked 상태 그대로 보존

**다음 작업 하나**: 현재 tracked 변경을 검토·배포한 뒤 `/`, `/control`,
`/settings`의 무캐시 응답과 실제 모바일 기기(390px/430px) 및 PC 브라우저의
실화면 사용성을 최종 검증한다.
