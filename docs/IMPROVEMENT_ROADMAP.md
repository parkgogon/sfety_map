...
- [x] 5. `/`, `/control`, `/settings` 경로를 분리하고, 경로 전환 직후 최신 화면을
  안정적으로 표시하도록 현재는 정적 import 단일 번들을 사용한다. 초기 번들
  크기는 프로덕션 빌드에서 확인한다.
...
- [x] 3. 대상 분석·전파의 모바일 선택·작업 흐름과 시설 목록을 정리한다.
- [x] 4. 위험도 정책 설정을 모바일 특보 카드 편집 방식으로 정리한다.
- [ ] 5. 390px·430px 실제 스마트폰과 PC 브라우저에서 배포 실화면을 최종
  확인한다. 반응형 코드·단위 테스트·빌드와 UX·디자인 문서화는 완료했다.

진행률: **구현·자동 검증 완료, 배포 실화면 검증 1건 남음**

## 4. 이번 작업 순서

**개선 로드맵 1~9단계 및 10단계 구현·자동 검증 완료**:
...
- 10단계: 모바일 UI 밀도 개선 구현·자동 검증 완료. 실제 390px·430px 스마트폰과
  PC 배포 실화면 확인은 남음
...
- [x] 10단계 모바일 UI 밀도 개선 (1차): 현장 지도 범례 `위험등급 1행 + 특보구역 1행` 분리, 미선택 안내판 높이 축소, 불필요한 주의 문구 정리
- [x] 10단계 모바일 UI 밀도 개선 (2차): 중앙관제 운영 상황 KPI를 전체·영향 우선 비조작형 요약 패널로 개편, 4개 등급 1줄 압축
- [x] 10단계 모바일 UI 밀도 개선 (3차): 대상 분석·전파 모바일 작업 순서 최적화(선택 요약 → 빠른 선택 → PDF/전파 → 목록 → 지도), 모바일 대상 선택 카드 도입
- [x] 10단계 모바일 UI 밀도 개선 (4차): 위험도 정책 설정 모바일 반응형 특보 카드(주의보/경보/중대 3열) 전환, 하단 액션 바 모바일 44px 이상 2열 배치
- [x] 10단계 모바일 UI 밀도 개선 (5차): `UI_FLOW.md` 및 `DESIGN_SYSTEM.md`
  반응형 문서화, Firestore TTL 테스트 안정화, 원클릭 5단계 통합 검증은 완료.
  실제 390px·430px 스마트폰과 PC 배포 실화면 확인은 남음

## 4. 향후 검토 과제: 목적 지향적 대대적 구조 개편 (Backlog)

사용자와의 화면 점검 및 요구사항 검토를 거쳐 보관된 중장기 핵심 아키텍처 개편 과제입니다.

### 1) 데스크탑 중앙관제 통합 관제 콕핏 (Split-View Cockpit)
* **목적**: 상황실 대형 화면/PC에서 지도와 점검 대상 목록을 탭 전환 없이 한눈에 모니터링하여 실시간 상황 판단력 극대화.
* **상세 구조**:
  - 좌측 60%: 대형 통합 관제 지도 (소관시설 103개소 실시간 마커 + KMA 특보 폴리곤 + 바람/강수/기온 레이어).
  - 우측 40%: 관제 상황판 & 액션 패널 (상단 요약 지표, 중단 점검 우선순위 TOP 시설 리스트, 하단 즉시 전파 및 PDF 다운로드 패널).
  - 양방향 인터랙션(Bi-directional Sync): 목록 시설 클릭 시 좌측 지도 해당 위치로 부드러운 패닝/줌, 지도 마커 클릭 시 우측 목록에서 해당 시설 스크롤 하이라이트.

### 2) 현장 시설담당자 스마트폰 PWA (Progressive Web App) 구축
* **목적**: 현장 요원이 모바일 브라우저 주소창 입력 없이 스마트폰 홈 화면에서 1초 만에 앱처럼 실행하고, 네트워크 불안정 환경에서도 비상연락망과 안전요령을 즉각 확인.
* **상세 구조**:
  - `manifest.json` 및 Service Worker 기반 스마트폰 홈 화면 설치 지원.
  - 산간 오지 등 통신 두절 환경에서도 103개 소관시설 기본 정보, 비상연락망, 재난별 핵심 안전관리요령을 열람할 수 있는 오프라인 정적 캐시 아키텍처 도입.

### 3) 복합 재난(Multi-Hazard) 동시 발생 시설의 가중치 시각화
* **목적**: 여름/가을철 폭염+호우, 태풍+강풍 등 2개 이상의 특보가 동시 발효된 시설의 복합재난 위험도를 직관적으로 인지.
* **상세 구조**:
  - 다중 특보 중첩 시설에 대한 '복합위험' 뱃지 부여.
  - 관제 우선순위 알고리즘에 복합재난 가중치(Multiplier)를 반영하여 점검 목록 최상단 자동 승격 및 전용 필터 지원.
