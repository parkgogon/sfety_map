...
(`TelegramOutboxService`), SMS 준비·상한·대체 전파(`SmsDeliveryService`),
잔액·정기 운영보고(`AlertOperationsService`)를 분리한다. 배포 서비스와
Scheduler 경로, Firestore 문서 형식, 자동알림 기준 상태는 기존과 동일하다.
...
`monitoring_snapshot_health=ERROR`만 남긴다.

공개 사용자 API와 API의 수동 상황전파는 현재 정책 버전과 같고 15분 이내에
생성된 최신 snapshot을 우선 읽는다. 저장본이 없거나 오래됐거나, 정책 버전이
다르거나, Firestore 조회·형식 검증이 실패하면 기존 KMA 직접 조회로 복귀한다.
오래된 정상 저장본만 남은 상태에서 KMA까지 실패하면 그 저장본을 `STALE`로
