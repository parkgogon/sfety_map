...
> 상태: 1~3단계 구현·운영 배포 완료, 4단계 자동 검증·문서 완료 / 실화면 확인 대기
>
> 작성 기준: 2026-08-30, `main` `431486c`, tracked working tree clean
>
> 배포 기준: 기능 커밋 `a34bbda`, GitHub Actions `33281586514` 성공
>
> 남은 범위: 인앱 브라우저의 PC·모바일 운영 실화면 확인만 수행한다. 사용자
> 확인 전에는 이 계획서를 삭제하거나 실화면 검수를 완료로 표시하지 않는다.
...
- 현재 세션에는 연결 가능한 브라우저가 없어 390×844·1440×900, level 7~10의
  ...
  아직 삭제하지 않으며 사용자 확인까지 남긴다.
- 기능 커밋 `a34bbda`의 GitHub Actions `33281586514`에서 test, Cloud Run
  API·Worker·5분 Scheduler, Firebase Hosting, uptime monitoring이 모두 성공했다.
  2026-08-30 08:47 KST에 운영 `/`, `/control`, `/settings`, 두 health 경로가 모두
  HTTP 200임을 확인했다.
...
- 구현·자동 검증·배포는 완료됐으므로 계획을 다시 만들거나 코드를 수정하지 않는다.
- 남은 작업은 인앱 브라우저의 운영 실화면 검수다. 이상이 있으면 증거 캡처·재현
  절차·영향 범위만 보고하고 사용자의 수정 요청 없이 구현을 시작하지 않는다.
- 관리자 인증을 우회하거나 실제 Telegram 전파·PDF 생성·설정 변경 같은 외부 상태
  변경을 수행하지 않는다.
- 사용자가 실화면 결과를 확인한 뒤에만 결과를 HANDOFF에 옮기고 이 임시
  계획서를 정리한다.

## 6. 인앱 브라우저 실화면 검수 시 다음 AI 실행 문구

```text
AGENTS.md, docs/HANDOFF.md와 docs/WIND_PARTICLE_PANNING_PLAN.md를 끝까지 읽고 현재
Git·배포 상태와 짧게 대조해줘. 기능 커밋 a34bbda와 배포는 완료됐으니 코드를
수정하지 말고 인앱 브라우저로 https://keco-safety-map.web.app의 PC 1440×900과
모바일 390×844 운영 실화면만 검수해줘. 현장지도·중앙관제, 실시간·모의훈련,
기온·강수·바람에서 계획서의 짧은·긴·연속 panning, zoom, 마커·클러스터 클릭,
범례·중립색 파티클·약풍/강풍 차이·잔상·끊김을 확인해줘. 실제 전파·설정 변경은
하지 마. 결과는 항목별 통과/실패, viewport·URL·모드·레이어, 증거 캡처와 재현 절차로
보고하고, 관리자 인증이 막히면 우회하지 말고 미확인으로 구분해줘.
```
