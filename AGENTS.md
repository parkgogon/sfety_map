...
- 한두 파일의 작은 수정이나 명확한 버그 수정은 불필요하게 나누지 않습니다.
- 여러 화면·서버·운영 경로를 함께 바꾸거나 한 세션을 넘길 가능성이 있는 작업은
  먼저 **계획 단계와 구현 단계를 분리**합니다. 계획 요청을 받은 차례에는 현황,
  목표·제외 범위, 2~5개의 구현 단계, 단계별 수용 조건과 테스트를 확정하고 코드는
  수정하지 않습니다.
- 이미 승인된 계획이 있으면 다음 작업자가 처음부터 다시 계획하지 않습니다. 실제
  코드·테스트·Git과 짧게 재대조한 뒤 아직 끝나지 않은 첫 단계부터 진행합니다.
- 장기 작업 계획은 필요할 때만 `docs/*_PLAN.md`로 저장하고 `HANDOFF.md`의
  `다음 작업`에서 연결합니다. 계획에는 작성 기준 commit/working tree, 현재 사실,
  단계별 범위·수용 조건·검증, 보류 결정을 적습니다.
- 구현은 한 번에 한 단계만 진행합니다. 다음 단계로 넘어가기 전에 해당 단계의
  diff를 검토하고 관련 테스트를 통과시켜 **독립적으로 설명 가능한 안정 상태**를
  만듭니다. 파일별·함수별로 지나치게 잘게 쪼개지는 않습니다.
- AI 교체, 세션 종료 또는 토큰 부족이 예상되면 새 단계를 시작하지 않습니다.
  현재 단계를 가능한 한 검증 가능한 경계까지 마친 뒤 계획과 `HANDOFF.md`에
  완료 범위, 남은 첫 작업, 변경 파일, 실행한 테스트와 실패를 사실대로 남깁니다.
  미완성 상태를 완료로 표시하지 않습니다.
- 커밋 요청을 받은 작업은 가능하면 검증된 단계 경계에서 커밋합니다. `main` push와
  배포는 별도 사용자 요청이 있을 때만 수행합니다.
- 구현이 끝난 임시 계획서는 현재 상태 문서처럼 남겨 두지 않습니다. 완료 결과는
  `HANDOFF.md`와 관련 설계 문서에 반영하고 계획서는 삭제하거나 보관합니다.

## 7. 안전한 작업 규칙

- 기존 dirty working tree와 사용자 산출물을 보존합니다. `output/`, `tmp/`,
  `.trash/`는 Git 제외 산출물·임시·보관 공간이므로 요청 없이
  삭제·추가·커밋하지 않습니다.
