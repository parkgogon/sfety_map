...
시설담당자 그룹 수동 전파 또는 PDF 버튼을 누를 때 한 번에 확정됩니다.
...
수동 상황전파는 관리자 API를 거쳐 시설담당자 Telegram 그룹으로 발송하며
